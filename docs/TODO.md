...
| Atribút                   | Hodnota                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| ------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Posledná aktualizácia** | 2026-08-30 (Atlas náklady — zrušený keep-warm cron `/health/ready` `*/4`, skutočná príčina nárastu 24,56→68,26 USD identifikovaná ako dedikovaný M10 cluster `inventario-dev` (prázdny, zmazaný), mŕtve CI secrets `MONGO_URI_TEST`/`ENTRA_*_TEST` odstránené, zdokumentované limity Flex záloh na produkcii — viď `docs/sessions/2026-08-30-atlas-naklady-keep-warm-cron.md`; predtým 2026-07-17 Zebra tlač v Chrome (LNA `loopback` + CORS simple request) a GlobalFetchOverlay prepnutý na `useIsMutating()` — tým uzavretá téma pomalého preloadera, session log dopísaný spätne; predtým 2026-07-16 (ADR-0036 „Vrátiť od osoby" — čiastočné a cross-loan vrátenie majetku, nová doplnková cesta popri nezmenenom tlačidle „Vrátiť", K1–K8 hotové, viď `docs/sessions/NEXT.md`; predtým opravené zamenené titulky HANDOVER/RETURN v tlačovom PDF protokole; predtým 2026-07-15 (ADR-0035 Fáza 1 implementovaná — org-aware /login cez ?org= hint, položka #26 uzavretá pre Fázu 1, Fáza 2 vlastná doména ostáva; predtým nová položka #26 — login stránka nezohľadňuje allowedAuthProviders organizácie, zaevidované bez fixu na Janikinu žiadosť; predtým cleanup „Osoby" po zlúčení — zmazané PersonsContent.tsx/PersonDetailContent.tsx/usePerson()/GET /v1/users/directory/:id, usePersonsDirectory zostáva kvôli Audit log filtru; oprava CI #365/#366 — testy neboli aktualizované po rozšírení toManagerShape() o firstName/lastName — viď `docs/sessions/NEXT.md`; predtým pomalé prekliky Používatelia → detail → majetok — diagnostikované ako súbežné API volania na stránku narážajúce na chýbajúci Fluid Compute, fix: `"fluid": true` vo vercel.json + nový MongoDB index `organisationId_borrowerId_createdAt` na loans — viď `docs/sessions/NEXT.md`; predtým detail + editácia používateľa na `/users/[id]` — nová read-only detail stránka s vypožičaným majetkom pre ASSET_MANAGER+ADMIN, samostatný editačný modál pre ADMIN (Meno/Priezvisko/Email pri LOCAL účte/rola/aktivita), K1–K4 hotové — viď `docs/sessions/NEXT.md`; predtým zlúčenie „Osoby" + „Používatelia" do jednej stránky /users, K1–K4 hotové a overené live; pri deployi odhalený a opravený samostatný bug — Vercel ignoreCommand pri multi-commit push preskočil build API — viď `docs/sessions/NEXT.md`; K5 cleanup čaká na Janikino potvrdenie; predtým pomalý preloader po nečinnosti — diagnostikované (cold-start serverless instancií) + fix nasadený (staleTime pre referenčné dáta + Vercel Cron keep-warm); predtým ADR-0034 — predpríprava budúceho používateľa pre DOMAIN_RESTRICTED organizácie, K1–K5 hotové, K6 openapi/api-types regen ostáva ako lokálny krok; predtým QR obrázok s logom, dependabot PR triage + prod incident `ERR_REQUIRE_ESM` (vyriešené), ADR-0027 write path gap doplnený, Zebra UI prepínač + záložky v Nastaveniach organizácie; backlog nižšie beze zmeny — ad-hoc feature požiadavky mimo formálny backlog)) |
...
- **Stav:** ✅ Fáza 1 DONE (2026-07-15) — riešené cez ADR-0035
  ...
  endpoint `GET /v1/public/organisations/login-context` (slug alebo
  domain) + `LoginPage.tsx` teraz číta `?org=<slug>` a filtruje
...
- **Overené pri nahlásení:** Janika otvorila plain `/login` (bez org-špecifického linku); videla nesprávne tlačidlá zobrazené, neklikala na ne — teda nepotvrdené/nevyvrátené naživo, či by klik skutočne prešiel (podľa kódu by nemal, backend gate by ho zamietol).
- **Súvisiaci menší nedostatok:** `ERROR_MESSAGES` v `LoginPage.tsx` nemá záznam pre `provider_not_allowed`/`entra_tenant_mismatch`, padá na všeobecné „Nastala chyba. Skúste znova." — aj keby niekto klikol a bol zamietnutý, nevidel by prečo.
- **Možné riešenia (z ADR-0031, nerozhodnuté ktoré):** (a) `?org=<slug>` hint v URL/invite linku, ktorý login stránka použije na filtrovanie tlačidiel, (b) email-first krok (najprv email → podľa domény ukázať len povolený provider), (c) tenant subdoména.
- **Blocker:** NIE — rieši sa, keď bude čas alebo keď to začne reálne miasť SFZ používateľov pri onboardingu.
- **Model:** Sonnet (frontend UX + drobný backend routing), Opus ak sa otvorí architektonická otázka (subdoména vs query hint).
...
### 27. Drobný úklid nájdený pri audite 2026-08-30

- **Stav:** evidované 2026-08-30. Žiadny blocker, všetko menšie než hodina práce.
- **Rozsah:**
  - [ ] `apps/web/src/lib/api-hooks.ts` — zrušiť dočasné pretypovanie
        `apiClient.GET/POST` pre `useBorrowerBorrowedItems` /
        `useReturnItemsFromBorrower`. Bolo potrebné len kým `api-types.ts`
        nepoznalo ADR-0036 endpointy; po `generate:api-types` už nie.
  - [ ] `.github/workflows/ci.yml` — `if: github.actor != 'dependabot[bot]'`
        na test jobe už nemá opodstatnenie (testy nepotrebujú žiadne
        secrets, `tests/setup.ts` MONGO_URI bezpodmienečne prepíše).
        Zrušením by dependabot PR bežali aj testami.
  - [ ] `.github/workflows/ci.yml` — triggery obsahujú
        `branches: [main, develop]`, ale vetva `develop` v repe neexistuje
        (ani lokálne, ani na origine). Mŕtvy odkaz.
  - [ ] Lokálna vetva `dependabot/npm_and_yarn/minor-and-patch-0db1d162ac`
        — remote už neexistuje (zlúčená a zmazaná na GitHube), lokálna
        kópia zostala. **Nemazať bez potvrdenia.**
  - [ ] **5 otvorených dependabot PR** na origine: `eslint/js-9.39.5`,
        `fastify-plugin-6.0.0`, `fastify/rate-limit-11.1.0`,
        `fastify/swagger-ui-6.1.0`, `minor-and-patch-1cd98fa4cd`.
        ⚠️ **Po každom zlúčení skontrolovať `get_runtime_errors` na
        produkcii do 1–2 minút, nielen CI status** — poučenie z incidentu
        `ERR_REQUIRE_ESM` (`@fastify/cookie` 11.1.1 → ESM-only `cookie@2.0.1`,
        plný výpadok API na každom cold starte).
  - [ ] Atlas: prekontrolovať projekt `contineo.app` — faktúra skočila
        11,50 → 65,20 USD, rovnaký podpis dedikovaného M10 ako mal
        `inventario-dev`.
  - [ ] GitHub Settings → Secrets: zmazať už nepoužívané `MONGO_URI_TEST`,
        `ENTRA_API_CLIENT_ID_TEST`, `ENTRA_TENANT_ID_TEST`.
  - [ ] Vercel `inventario-api` → Preview `MONGO_URI` ukazuje na zmazaný
        cluster — buď premennú odstrániť, alebo Preview deploye vedome
        nechať padať pri štarte.
- **Model:** Haiku/Sonnet podľa položky.

### 28. DR test záloh + zosúladenie compliance dokumentov

- **Stav:** otvorené 2026-08-30 (náväznosť na starší bod „smoke + DR test"
  z júna, ktorý nikdy nebehal).
- **Kontext:** `inventario-prod` je Atlas **Flex** → zálohy sú **8 denných
  snapshotov**, bez vlastnej politiky, bez on-demand snapshotov a **bez
  Point-in-Time restore**. Reálne RPO až 24 h. Vedome ponechané na Flexe
  (M10 by stálo ~58 USD/mes.). Detail:
  `docs/sessions/2026-08-30-atlas-naklady-keep-warm-cron.md` (dodatok 2).
- **Čo treba:**
  1. Overiť v Atlas → `inventario-prod` → Backup, že snapshoty naozaj
     existujú. Zálohu, ktorú nikto nikdy nevidel, nemáme.
  2. Skúsiť restore nanečisto (do iného clustera alebo lokálny download).
     Prvý restore nesmie byť až v deň, keď o dáta reálne prídeme.
  3. Skontrolovať, či `docs/compliance/` (Data Retention Schedule,
     Information Security Policy, DPIA reference pack, Security
     Whitepaper) netvrdí o zálohovaní a obnove viac, než Flex poskytuje.
- **Model:** Opus (compliance formulácie), Janika (Atlas kliky).
- **Blocker:** NIE pre pilot, ale je to najväčšie neotestované riziko na
  ostrých dátach.
